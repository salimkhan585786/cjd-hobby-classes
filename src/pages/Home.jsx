            ...
            >
              ...
              <div className="grid gap-10 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
                <div className="max-w-2xl space-y-6">
                  <motion.span initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="inline-flex items-center gap-2 rounded-full bg-violet-500/15 px-4 py-2 text-sm uppercase tracking-[0.28em] text-violet-200">
                    New season enrollment
                  </motion.span>
                  <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.7, ease: [0.22, 1, 0.36, 1] }} className="text-5xl font-semibold leading-tight text-white sm:text-6xl">
                    Modern art classes for students who want skill, confidence, and a beautiful portfolio.
                  </motion.h1>
                  <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }} className="max-w-xl text-lg leading-8 text-slate-300">
                  ...
                  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.45 }} className="flex flex-col gap-3 sm:flex-row">
                    <MagneticButton className="inline-flex items-center justify-center rounded-full bg-violet-500 px-6 py-4 text-base font-semibold text-white transition hover:bg-violet-400">
                      <Link to="/register">Join Classes</Link>
                    </MagneticButton>
                    <MagneticButton className="inline-flex items-center justify-center rounded-full border border-white/10 bg-slate-900/80 px-6 py-4 text-base text-slate-100 transition hover:bg-slate-900">
                      <Link to="/workshops">Book Workshop</Link>
                    </MagneticButton>
                    <MagneticButton className="inline-flex items-center justify-center rounded-full border border-violet-500 bg-violet-500/15 px-6 py-4 text-base text-violet-100 transition hover:bg-violet-500/25">
                    ...
                    <div className="grid gap-5">
                      ...
                      <div className="grid gap-5 sm:grid-cols-2">
                        <div className="glass-card rounded-[2rem] p-5 shadow-soft">
                          <div className="flex items-center gap-3 text-violet-300"><Brush size={20} /><span className="text-sm uppercase tracking-[0.24em]">Course paths</span></div>
                          <p className="mt-4 text-xl font-semibold text-white">From pencil foundations to digital illustration</p>
                        </div>
                        <div className="glass-card rounded-[2rem] p-5 shadow-soft">
                          <div className="flex items-center gap-3 text-violet-300"><Sparkles size={20} /><span className="text-sm uppercase tracking-[0.24em]">Class management</span></div>
        ...
        <StaggerContainer className="grid gap-6 lg:grid-cols-3">
          {[
            ...
            <StaggerItem key={item.title}>
              <div className="glass-card rounded-[2rem] border border-white/10 p-6 shadow-soft transition-all duration-300 hover:-translate-y-1 hover:shadow-glow">
                <div className="flex items-center gap-3 text-violet-300"><Star size={18} /><span className="text-sm uppercase tracking-[0.24em]">Student success</span></div>
        ...
        <FadeInView>
          <section className="space-y-8">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
              ...
              <Link to="/courses" className="rounded-full border border-white/10 px-5 py-3 text-sm text-slate-200 transition hover:bg-white/5">Explore all courses</Link>
            </div>
        ...
        <FadeInView>
          <section className="space-y-10">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
              ...
              <Link to="/workshops" className="rounded-full bg-violet-500 px-5 py-3 text-sm font-semibold text-white transition hover:bg-violet-400">See all workshops</Link>
            </div>
            <div className="grid gap-6 lg:grid-cols-3">
              {workshopsLoading
                ? Array.from({ length: 3 }).map((_, i) => <LoadingSkeleton key={i} className="h-72" />)
                : workshops.map((workshop, idx) => <WorkshopCard key={workshop.id} workshop={workshop} variant={['violet', 'fuchsia', 'pink'][idx % 3]} />)}
            </div>
          </section>
        </FadeInView>
      </ParallaxSection>
            ...
            <div className="glass-card rounded-[2rem] border border-white/10 p-10 shadow-soft">
              ...
              <div className="mt-8 grid gap-6">
                {valueProps.map((item, index) => (
                  <motion.div key={item.title} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.15 }} className="rounded-3xl bg-slate-950/80 p-6 shadow-soft transition-transform duration-300 hover:translate-x-1">
              ...
              {testimonials.map((item) => (
                <StaggerItem key={item.name}>
                  <TiltCard tiltAmount={8}>
                    <article className="glass-card rounded-[2rem] border border-white/10 p-8 shadow-soft">
                      <p className="text-lg leading-8 text-slate-200">&ldquo;{item.feedback}&rdquo;</p>
      ...
      <motion.div ref={ctaRef} style={{ y: ctaY }}>
        <FadeInView>
          <RotateParallax rotateRange={[-2, 2]} axis="both">
            <section className="relative overflow-hidden rounded-[3rem] border border-white/10 bg-gradient-to-r from-violet-500/15 via-fuchsia-500/10 to-cyan-500/10 p-10 shadow-soft">
              ...
              <div className="relative flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
                ...
                <div className="flex flex-col gap-3 sm:flex-row">
                  <MagneticButton className="rounded-full bg-violet-500 px-6 py-4 text-center font-semibold text-white transition hover:bg-violet-400">
                    <Link to="/register">Create student account</Link>
                  </MagneticButton>
                  <MagneticButton className="rounded-full border border-white/10 px-6 py-4 text-center text-slate-100 transition hover:bg-white/5">
